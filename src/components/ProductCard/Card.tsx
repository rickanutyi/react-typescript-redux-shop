import React, { FC } from "react";
import { useNavigate } from "react-router-dom";
import { DefaultStateProducts, ProductType } from "../../store/reducers/types";
import "./style/Card.css";

type Props = {
  product: ProductType;
};
const Card: FC<Props> = ({ product }) => {
  const navigate = useNavigate();
  return (
    <div className="card">
      <div
        onClick={() => navigate(`/details/${product.id}`)}
        className="card_image"
      >
        <img src={product !== null ? product.images[1] : ""} alt="image" />
      </div>
      <div className="preview_title">
        {product !== null ? product.name : null}
      </div>
      <div className="card_actions">
        <div className="old_price">135р</div>
        <div className="preview_price">
          {product !== null ? product.price : "..."}
        </div>
        <div className="btn_to-basket">
          В корзину
          <svg
            className="basket"
            xmlns="http://www.w3.org/2000/svg"
            width="20"
            height="20"
            viewBox="0 0 24 24"
          >
            <path d="M16.53 7l-.564 2h-15.127l-.839-2h16.53zm-14.013 6h12.319l.564-2h-13.722l.839 2zm5.983 5c-.828 0-1.5.672-1.5 1.5 0 .829.672 1.5 1.5 1.5s1.5-.671 1.5-1.5c0-.828-.672-1.5-1.5-1.5zm11.305-15l-3.432 12h-13.017l.839 2h13.659l3.474-12h1.929l.743-2h-4.195zm-6.305 15c-.828 0-1.5.671-1.5 1.5s.672 1.5 1.5 1.5 1.5-.671 1.5-1.5c0-.828-.672-1.5-1.5-1.5z" />
          </svg>
        </div>
      </div>
    </div>
  );
};

export default Card;
